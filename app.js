...
function abrirWhatsappComprovante(){ if(!apostaComprovanteAtual){ alert("Nenhum comprovante aberto."); return; } const texto=montarMensagemWhatsappComprovante(apostaComprovanteAtual); window.open(`https://wa.me/?text=${encodeURIComponent(texto)}`,"_blank","noopener"); }
...
function loginAdmin(){ const u=document.getElementById("usuarioAdmin"), s=document.getElementById("senhaAdmin"), l=document.getElementById("adminLogin"), area=document.getElementById("adminArea"); if(!u||!s||!l||!area)return; if(u.value.trim()!==USUARIO_ADMIN_TESTE||s.value!==SENHA_ADMIN_TESTE){ alert("Usuario ou senha incorretos."); return; } area.classList.remove("oculto"); l.classList.add("oculto"); u.value=""; s.value=""; mostrarAbaAdmin("painel"); aplicarConfigBolaoNaTela(); renderAdmin(); }
function sairAdmin(){ const u=document.getElementById("usuarioAdmin"), s=document.getElementById("senhaAdmin"), l=document.getElementById("adminLogin"), area=document.getElementById("adminArea"); if(!u||!s||!l||!area)return; area.classList.add("oculto"); l.classList.remove("oculto"); u.value=""; s.value=""; u.focus(); }
